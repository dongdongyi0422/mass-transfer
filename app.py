...
# =====================================================================
# ========== UI HELPERS (solvent velocity 버그 완전 수정됨) ============
# =====================================================================

def nudged_slider(label, vmin, vmax, vstep, vinit, key,
                  unit="", decimals=3, help=None):
...
def log_slider(label, exp_min, exp_max, exp_step, exp_init, key,
               unit="", help=None):
    ...
    st.slider(
        lab, float(exp_min), float(exp_max),
        float(st.session_state[key]),
        float(exp_step),
        help=help,
        key=f"{key}__s",
...
def D_temp_correction(D_ref: float, T_ref: float, T: float,
                      eta_ref: float, eta: float) -> float:
...
def membrane_side_conc_from_donnan(c_bulk, z_map, K_map, psi, T):
    out = {}
    for sp in c_bulk:
        out[sp] = K_map.get(sp,1.0)*c_bulk[sp] * np.exp(-(z_map[sp]*F*psi)/(R*T))
    return out
...
def run_ion_membrane():
    st.header("Ion-Exchange / NF Membrane — NP + Donnan")

    with st.sidebar:
        st.subheader("Membrane & Operation")
        T   = nudged_slider("Temperature", 273.15, 360.0, 0.5, 298.15,
                            key="T_i", unit="K")
        Lnm = nudged_slider("Active layer thickness", 10.0, 5000.0, 1.0, 200.0,
                            key="Lnm_i", unit="nm")
        eps = nudged_slider("Porosity ε", 0.05, 0.8, 0.01, 0.30,
                            key="eps_i", unit="–")
        tau = nudged_slider("Tortuosity τ", 1.0, 5.0, 0.1, 2.0,
                            key="tau_i", unit="–")
        Cf  = nudged_slider("Fixed charge C_f", -3000.0, 3000.0, 10.0, -500.0,
                            key="Cf_i", unit="mol/m³")
        dV  = nudged_slider("Applied potential ΔV", -0.3, 0.3, 0.005, 0.05,
                            key="dV_i", unit="V")

        # ★ solvent velocity (버그 수정됨)
        v_s = nudged_slider("Solvent velocity v", -1e-6, 1e-6, 1e-7, 0.0,
                            key="vsolv_i", unit="m/s")

        st.subheader("Species set")
        all_cations = [k for k in ION_DB if ION_DB[k]["z"]>0]
        all_anions  = [k for k in ION_DB if ION_DB[k]["z"]<0]
        sel_cat = st.multiselect("Cations", all_cations, default=["Na+","K+"],
                                 key="sel_cat_i")
        sel_an  = st.multiselect("Anions",  all_anions,  default=["Cl-","OH-"],
                                 key="sel_an_i")

        st.subheader("Bulk concentrations (mol/m³)")
        c_feed = {}; c_perm = {}
        for sp in sel_cat + sel_an:
            c_feed[sp] = nudged_slider(f"{sp} feed", 0.0, 2000.0, 1.0, 100.0,
                                      key=f"cf_{sp}", unit="mol/m³")
            c_perm[sp] = nudged_slider(f"{sp} permeate", 0.0, 2000.0, 1.0, 10.0,
                                       key=f"cp_{sp}", unit="mol/m³")
        ...
        if auto_kd:
            ...
            D_map = {}
            for sp in (sel_cat + sel_an):
                D_map[sp] = D_temp_correction(ION_DB[sp]["D"], T_ref, T,
                                              eta_ref, eta_now)
            ...
            D_map = {}
            for sp in (sel_cat + sel_an):
                K_map[sp] = nudged_slider(f"K {sp}", 0.01, 10.0, 0.01, 1.0,
                                          key=f"K_{sp}")
                D_map[sp] = log_slider(f"D {sp}", -11.0, -8.0, 0.1,
                                       np.log10(ION_DB[sp]["D"]),
...
# =====================================================================
# MODE 1 — GAS MEMBRANE (Original UI, 3D plot 삭제됨)
# =====================================================================
...
def effective_diameter_A(gas, T, alpha):
    dA = GAS_PARAMS[gas]["d"]
    lamA = de_broglie_lambda_m(T, GAS_PARAMS[gas]["M"]) * 1e10
...
def dsl_loading_and_slope_b(gas,T,Pbar,relP,q1,q2,b1,b2):
    ...
    q1_molkg=q1*1e-3*1e3
    q2_molkg=q2*1e-3*1e3

    q_vec=np.zeros_like(relP,float)
    dqdp=np.zeros_like(relP,float)
...
def run_gas_membrane():
    ...
    with st.sidebar:
        T    = nudged_slider("Temperature", 10.0, 600.0, 1.0, 300.0,
                             key="T_g", unit="K")
        Pbar = nudged_slider("Total Pressure", 0.1, 10.0, 0.1, 1.0,
                             key="Pbar_g", unit="bar")
        d_nm = nudged_slider("Pore diameter", 0.05, 50.0, 0.01, 0.36,
                             key="d_nm_g", unit="nm")
        L_nm = nudged_slider("Membrane thickness", 10.0, 100000.0, 1.0, 100.0,
                             key="L_nm_g", unit="nm")

        gases = list(GAS_PARAMS.keys())
        gas1  = st.selectbox("Gas1 (numerator)",   gases,
                             index=gases.index("CO2"), key="gas1_g")
        gas2  = st.selectbox("Gas2 (denominator)", gases,
                             index=gases.index("CH4"), key="gas2_g")

        st.subheader("DSL parameters (mmol/g, Pa⁻¹)")
        q11 = nudged_slider("q1 Gas1", 0.0, 100.0, 0.01, 1.50,
                            key="q11_g", unit="mmol/g")
        q12 = nudged_slider("q2 Gas1", 0.0, 100.0, 0.01, 0.50,
                            key="q12_g", unit="mmol/g")
        b11 = nudged_slider("b1 Gas1", 1e-10, 1e-1, 1e-8, 1e-5,
                            key="b11_g", unit="Pa⁻¹", decimals=8)
        b12 = nudged_slider("b2 Gas1", 1e-10, 1e-1, 1e-8, 5e-6,
                            key="b12_g", unit="Pa⁻¹", decimals=8)

        q21 = nudged_slider("q1 Gas2", 0.0, 100.0, 0.01, 1.00,
                            key="q21_g", unit="mmol/g")
        q22 = nudged_slider("q2 Gas2", 0.0, 100.0, 0.01, 0.40,
                            key="q22_g", unit="mmol/g")
        b21 = nudged_slider("b1 Gas2", 1e-10, 1e-1, 1e-8, 5e-6,
                            key="b21_g", unit="Pa⁻¹", decimals=8)
        b22 = nudged_slider("b2 Gas2", 1e-10, 1e-1, 1e-8, 1e-6,
                            key="b22_g", unit="Pa⁻¹", decimals=8)

        st.subheader("Quantum correction")
        alpha = nudged_slider("α (manual)", 0.0, 0.6, 0.01, 0.0,
                              key="alpha_g")

        rp_focus = nudged_slider("rₚ focus", 0.01, 0.99, 0.01, 0.35,
                                 key="rp_focus_g", unit="–")
        zoom_band = st.checkbox("Zoom band around rₚ focus",
                                value=False, key="zoom_band_g")

        if st.session_state.get("zoom_band_g", False):
            zoom_half = nudged_slider("Zoom half-width Δrₚ",
                                      0.01, 0.49, 0.01, 0.15,
    ...
    q1_mg, dqdp1 = dsl_loading_and_slope_b(gas1, T, Pbar,
                                           relP, q11, q12, b11, b12)
    q2_mg, dqdp2 = dsl_loading_and_slope_b(gas2, T, Pbar,
                                           relP, q21, q22, b21, b22)
    ...
    for i, rp in enumerate(relP):
        A = intrinsic_permeances(gas1, T, Pbar, d_nm, rp, L_nm,
                                 float(dqdp1[i]), alpha)
        B = intrinsic_permeances(gas2, T, Pbar, d_nm, rp, L_nm,
                                 float(dqdp2[i]), alpha)

        wA = weights_from_intrinsic(A)
        wB = weights_from_intrinsic(B)
    ...
    Sel     = np.divide(Pi1, Pi2, out=np.zeros_like(Pi1),
                        where=(Pi2 > 0))
    Pi1_gpu = Pi1 / GPU_UNIT
    Pi2_gpu = Pi2 / GPU_UNIT

    # =========== PLOTS (3D completely removed) ============

    # Mechanism band plot
    figBand, axBand = plt.subplots(figsize=(9, 0.7))
    axBand.imshow(rgba, extent=(relP[0], relP[-1], 0, 1),
                  aspect="auto", origin="lower")
    axBand.set_xlim(relP[0], relP[-1])
    axBand.set_yticks([])
    axBand.set_xlabel(r"Relative pressure, $P/P_0$ (–)")

    handles = [plt.Rectangle((0,0),1,1,fc=MECH_COLOR[n],
                             ec='none', label=n)
               for n in MECH_ORDER]
    leg = axBand.legend(handles=handles, loc="upper center",
...
def run_vascular_drug():
    ...
    with st.sidebar:
        Rv_um = nudged_slider("Vessel radius", 2.0, 100.0, 1.0, 4.0,
                              key="Rv_um_v", unit="μm")
        L_mm  = nudged_slider("Segment length", 1.0, 200.0, 1.0, 20.0,
                              key="Lv_mm_v", unit="mm")
        U     = nudged_slider("Mean velocity U", 0.1e-3, 20e-3, 0.1e-3, 1e-3,
                              key="U_v", unit="m/s")

        drug  = st.selectbox("Drug", list(DRUG_DB.keys()), index=0,
                             key="drug_v")

        T_body = nudged_slider("Temperature", 293.15, 312.15, 0.5,
                               310.15, key="Tdrug_v", unit="K")

        auto_Db = st.checkbox("Auto-compute D_b (T/η)",
                              value=True, key="autoDb_v")

        if auto_Db:
            T_ref = 298.15
            eta_ref = eta_water_PaS(T_ref)
            eta_now = eta_water_PaS(T_body)
            Db = D_temp_correction(DRUG_DB[drug]["Db"], T_ref, T_body,
                                   eta_ref, eta_now)
            st.caption(f"D_b auto: {Db:.3e} m²/s")
        else:
            Db = log_slider("D_b", -12.0, -8.0, 0.1,
                            np.log10(DRUG_DB[drug]["Db"]),
                            key="Db_v", unit="m²/s")

        Pv   = log_slider("P_v",   -9.0, -5.0, 0.1,
                          np.log10(DRUG_DB[drug]["Pv"]),
                          key="Pv_v", unit="m/s")

        kel  = log_slider("k_elim", -6.0, -2.0, 0.1,
                          np.log10(DRUG_DB[drug]["kel"]),
                          key="kel_v", unit="s⁻¹")

        C0   = nudged_slider("Reference conc. C₀", 0.0, 5.0, 0.01, 1.0,
                             key="C0_v", unit="mol/m³")

        profile = st.selectbox("Inlet profile",
                               ["Bolus (Gaussian)", "Constant infusion"],
                               index=0, key="pulse_v")

        t_end = nudged_slider("Sim time", 0.1, 600.0, 0.1, 60.0,
                              key="tend_v", unit="s")
        dt    = nudged_slider("Δt", 1e-3, 0.5, 1e-3, 0.01,
                              key="dt_v", unit="s")
