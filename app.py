# app.py — Membrane Transport Simulator (weighted mechanism maps; time-aware band)
# - Mechanism band uses weights (softmax or improved heuristic), NOT hard classify labels
# - In Time mode, band x-axis switches to seconds
...
# Optional: damp Knudsen dominance in softmax (helps band show Surface/Capillary)
DAMP_KNUDSEN = True
DAMP_FACTOR  = 1e-3  # 1e-2~1e-4 사이로 조절
...
def mechanism_weights(gas, other, T, P_bar, pore_d_nm, rp, dqdp_mkpa):
    # improved heuristic with mutual inhibition
    ...
    s = sum(w.values())
    if s <= 1e-12:
        return {"Blocked":0.0,"Sieving":0.0,"Knudsen":0.0,"Surface":1.0,"Capillary":0.0,"Solution":0.0}
    for k in w: w[k] /= s
    return w

def damp_knudsen_if_needed(Pi_intr: dict, d_nm: float, rp: float) -> dict:
    if DAMP_KNUDSEN and (d_nm >= 2.0 and rp >= 0.55):
        Pi_intr["Knudsen"] *= DAMP_FACTOR
    return Pi_intr

# ================== Mechanism bands (weighted) ==================
def mechanism_band_rgba(g1, g2, T, P_bar, d_nm, relP, L_nm,
                        q11,q12,b11,b12):
...
def mechanism_band_rgba_time(g1, g2, T, P_bar, d_nm, L_nm,
                             t_vec, P_bar_t, dqdp_series_g1, P0bar):
    L_m = max(float(L_nm), 1e-3) * 1e-9
    M1  = PARAMS[g1]["M"]
...
with colA:
    ...
    st.markdown(
        f"**Mechanism (rule):** `{rule_mech}`  |  **Best intrinsic (Gas1):** `{best_intrinsic}`"
    )
    st.caption(
        "Reference-only rule shown here. The band above uses weighted mechanisms.\n"
        + "Intrinsic Π (mid x-axis):  \n"
        + "  • " + "  \n  • ".join([f"{k}: {v:.3e} mol m⁻² s⁻¹ Pa⁻¹" for k,v in cand.items()])
    )

st.markdown("---")
st.caption(
    "Mechanism band uses weighted selection (softmax/heuristic). In Time mode the band’s x-axis is Time (s). "
    "Surface/Solution use DSL slope (∂q/∂p) via ρ_eff. GPU = Π / 3.35×10⁻¹⁰."
)
