...
# - Gas membrane: α(드브로이) 자동/수동 + λ,d_eff 표시, 메커니즘 밴드/퍼미언스/선택도 + 3D(A,B,D)
# - Ion membrane: Bulk conc sliders + K&D 자동(T/η) + 수동 오버라이드 옵션
#                 + crack 유도 열폭주 시뮬레이션(Transient)
...
def run_gas_membrane():
    ...
    with colB:
        ...
        if time_mode:
            ...
            rgba, _ = mechanism_band_rgba(gas1, gas2, T, Pbar, d_nm, X_vals, L_nm, q11, q12, b11, b12, alpha)
        ...
        # =================== 3D Plots (A + B + D) ===================
        st.markdown("---")
        st.subheader("3D views (A + B + D)")

        # ---- 3D A/B: Pi1 & Selectivity surfaces vs (P/P0, d_pore) ----
        # d-range: micropore-oriented sweep around 현재 d_nm (0.05–2.0 nm 범위에 클립)
        d_min_3d = max(0.05, d_nm * 0.3)
        d_max_3d = min(2.0, d_nm * 3.0)
        if d_max_3d <= d_min_3d:
            d_max_3d = d_min_3d + 0.05
        d_vals = np.linspace(d_min_3d, d_max_3d, 35)

        # rp-range: 0.01–0.99 균일
        rp3 = np.linspace(0.01, 0.99, 50)

        # DSL slope for 3D (gas1/gas2, 시간모드 여부와 무관하게 동일 DSL 사용)
        q1_3, dq1_3 = dsl_loading_and_slope_b(gas1, T, Pbar, rp3, q11, q12, b11, b12)
        q2_3, dq2_3 = dsl_loading_and_slope_b(gas2, T, Pbar, rp3, q21, q22, b21, b22)

        Z_Pi1 = np.zeros((len(d_vals), len(rp3)))
        Z_Sel = np.zeros_like(Z_Pi1)

        for i_d, d_val in enumerate(d_vals):
            Pi1_3 = permeance_series_SI(d_val, gas1, gas2, T, Pbar, rp3, L_nm, q1_3, dq1_3, q2_3, alpha)
            Pi2_3 = permeance_series_SI(d_val, gas2, gas1, T, Pbar, rp3, L_nm, q2_3, dq2_3, q1_3, alpha)
            sel_3 = np.divide(Pi1_3, Pi2_3, out=np.zeros_like(Pi1_3), where=(Pi2_3 > 0))
            Z_Pi1[i_d, :] = Pi1_3 / GPU_UNIT
            Z_Sel[i_d, :] = sel_3

        RP_grid, D_grid = np.meshgrid(rp3, d_vals)

        # 3D A — Pi1 surface
        fig3d_A = plt.figure(figsize=(7, 4))
        ax3d_A = fig3d_A.add_subplot(111, projection="3d")
        surfA = ax3d_A.plot_surface(RP_grid, D_grid, Z_Pi1,
                                    cmap="viridis", linewidth=0, antialiased=True)
        ax3d_A.set_xlabel(r"$P/P_0$ (–)")
        ax3d_A.set_ylabel("d_pore (nm)")
        ax3d_A.set_zlabel(f"{gas1} Π (GPU)")
        fig3d_A.colorbar(surfA, shrink=0.6, aspect=10, label="Π (GPU)")
        st.pyplot(fig3d_A, use_container_width=True); plt.close(fig3d_A)

        # 3D B — Selectivity surface
        fig3d_B = plt.figure(figsize=(7, 4))
        ax3d_B = fig3d_B.add_subplot(111, projection="3d")
        surfB = ax3d_B.plot_surface(RP_grid, D_grid, Z_Sel,
                                    cmap="viridis", linewidth=0, antialiased=True)
        ax3d_B.set_xlabel(r"$P/P_0$ (–)")
        ax3d_B.set_ylabel("d_pore (nm)")
        ax3d_B.set_zlabel(f"Selectivity {gas1}/{gas2}")
        fig3d_B.colorbar(surfB, shrink=0.6, aspect=10, label="Selectivity")
        st.pyplot(fig3d_B, use_container_width=True); plt.close(fig3d_B)

        # 3D D — Transient LDF: (t, P/P0(t), Π1(t))
        if time_mode:
            rp_t = np.clip(P_bar_t / float(st.session_state["P0bar_g"]), 1e-6, 0.9999)
            Pi1_gpu_t = Pi1 / GPU_UNIT

            fig3d_D = plt.figure(figsize=(7, 4))
            ax3d_D = fig3d_D.add_subplot(111, projection="3d")
            ax3d_D.plot3D(t, rp_t, Pi1_gpu_t, lw=1.5)
            ax3d_D.set_xlabel("t (s)")
            ax3d_D.set_ylabel(r"$P/P_0(t)$ (–)")
            ax3d_D.set_zlabel(f"{gas1} Π (GPU)")
            st.pyplot(fig3d_D, use_container_width=True); plt.close(fig3d_D)
        else:
            st.info("3D D (transient LDF) 플롯은 'Time (transient LDF)' 모드에서 활성화됩니다.")
    ...
    if enable_runaway:
        ...
        for k in range(1, nstep):
            ...
            D_T = {sp: D_temp_correction(ION_DB[sp]["D"], T_REF_ION, T_old, eta_ref, eta_now)
                   for sp in (sel_cat+sel_an)}
            P_crack = {sp: (D_T[sp]*eps/max(tau,1e-9))/max(L_loc,1e-12) * (1.0 + G_crk * f_old) for sp in (sel_cat+sel_an)}
            P_bulk  = {sp: (D_T[sp]*eps/max(tau,1e-9))/max(L_loc,1e-12) for sp in (sel_cat+sel_an)}
